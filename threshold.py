...
def create_threshold_map(jumps, pixel_range, MEDIAN_PIXEL, std_jump_dev):
    ...
    if MEDIAN_PIXEL * .93 <= middle_in_range <= MEDIAN_PIXEL * 1.07:
        dark_thresh_per = int(100 * (jumps[0]/float(lowest_in_range))) - 2  # - 8
        medium_thresh_per = int(100 * (jumps[1]/float(middle_in_range)))  # - 5
        light_thresh_per = int(100 * (jumps[2]/float(highest_in_range))) -4  # - 8
...
def get_local_avg_from_integral(integral_image, coords, trunk, box_size=140): # float()
    ...
    """ Send in a coordinate pair, return the integral average of that area"""
	...
	# if the box above goes off screen and fails, get the boxes down
        x_se, y_se = x, y
